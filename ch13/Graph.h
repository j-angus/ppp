...
//------------------------------------------------------------------------------
// arc,defined by centre, radius, beginning and ending angle, based on Circle
struct Arc_c : Shape {
	Arc_c(Point p, int rr, int ss, int ff)	// center, radius, start and finish angles
		:r(rr), s(ss), f(ff) { add(Point(p.x-rr,p.y-rr)); }
...
private:
	int r; // radius
	int s; // start angle 0 - 360
	int f; // finish angle 0 -360
};
...
struct Arc : Shape { // center, min, and max distance from center, start & finish angles
	Arc(Point p, int ww, int hh, int ss, int ff)
		:w(ww), h(hh), s(ss), f(ff) { add(Point(p.x-ww,p.y-hh)); }
